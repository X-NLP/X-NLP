    ...
    void appliesVersionedMigrations() {
        ...
        assertThat(history).extracting(row -> row.get("DESCRIPTION"))
                .containsExactly("baseline", "evaluation-progress-columns", "waste-weighing-trip-number",
                        ...
                        "dataset-versioning", "resumable-evaluation", "pipeline-dag");
        ...
        assertThat(jdbc.queryForObject(
                ...
                .isGreaterThanOrEqualTo(0);
        assertThat(jdbc.queryForObject(
                "SELECT COUNT(*) FROM evaluation_recovery_runs WHERE tenant_id = 'default'", Integer.class))
                .isGreaterThanOrEqualTo(0);
        assertThat(jdbc.queryForObject(
                "SELECT COUNT(*) FROM evaluation_sample_results WHERE tenant_id = 'default'", Integer.class))
                .isGreaterThanOrEqualTo(0);
        assertThat(jdbc.queryForObject(
                "SELECT COUNT(*) FROM pipeline_definitions WHERE tenant_id = 'default'", Integer.class))
                .isGreaterThanOrEqualTo(0);
        assertThat(jdbc.queryForObject(
                "SELECT COUNT(*) FROM pipeline_runs WHERE tenant_id = 'default'", Integer.class))
                .isGreaterThanOrEqualTo(0);
        assertThat(jdbc.queryForObject(
                "SELECT COUNT(*) FROM pipeline_run_events WHERE tenant_id = 'default'", Integer.class))
                .isGreaterThanOrEqualTo(0);
    }
}
