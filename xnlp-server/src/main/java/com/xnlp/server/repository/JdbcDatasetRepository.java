...
public class JdbcDatasetRepository implements DatasetRepository {
    ...
    public List<EvaluationDataset> findAll() {
        return jdbc.query("""
                ...
                FROM datasets WHERE tenant_id = ? ORDER BY updated_at DESC, name
                """, this::mapDataset, TenantContext.currentTenantId());
    }
    ...
    public Optional<EvaluationDataset> findById(String id) {
        List<EvaluationDataset> datasets = jdbc.query("""
                SELECT id, name, description, task_type, entry_count, created_at, updated_at
                FROM datasets WHERE id = ? AND tenant_id = ?
                """, this::mapDataset, id, TenantContext.currentTenantId());
        if (datasets.isEmpty()) return Optional.empty();
        EvaluationDataset dataset = datasets.getFirst();
        dataset.setEntries(loadEntries(id, TenantContext.currentTenantId()));
    ...
    public EvaluationDataset save(EvaluationDataset dataset) {
        ...
        int updated = jdbc.update("""
                UPDATE datasets SET name = ?, description = ?, task_type = ?, entry_count = ?,
                    created_at = ?, updated_at = ? WHERE id = ? AND tenant_id = ?
        ...
        if (updated == 0) {
            jdbc.update("""
                    INSERT INTO datasets (id, tenant_id, name, description, task_type, entry_count, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, dataset.getId(), TenantContext.currentTenantId(), dataset.getName(), dataset.getDescription(),
                    dataset.getTaskType() == null ? null : dataset.getTaskType().name(), entries.size(),
                    Timestamp.from(dataset.getCreatedAt()), Timestamp.from(dataset.getUpdatedAt()));
        }
        jdbc.update("DELETE FROM dataset_entries WHERE dataset_id = ? AND tenant_id = ?",
                dataset.getId(), TenantContext.currentTenantId());
        for (int i = 0; i < entries.size(); i++) {
            ...
            entry.setId(entryId);
            jdbc.update("""
                    INSERT INTO dataset_entries (id, tenant_id, dataset_id, seq, input_text, expected_output, labels_json, metadata_json)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, entryId, TenantContext.currentTenantId(), dataset.getId(), i, entry.getInput(), entry.getExpectedOutput(),
    ...
    public void deleteById(String id) {
        jdbc.update("DELETE FROM dataset_entries WHERE dataset_id = ? AND tenant_id = ?",
                id, TenantContext.currentTenantId());
        jdbc.update("DELETE FROM datasets WHERE id = ? AND tenant_id = ?",
                id, TenantContext.currentTenantId());
    }

    @Override
    public int count() {
        Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM datasets WHERE tenant_id = ?",
                Integer.class, TenantContext.currentTenantId());
    ...
    private List<EvaluationEntry> loadEntries(String datasetId, String tenantId) {
        return jdbc.query("""
                SELECT id, input_text, expected_output, labels_json, metadata_json
                FROM dataset_entries WHERE dataset_id = ? AND tenant_id = ? ORDER BY seq
                """, (rs, rowNum) -> {
