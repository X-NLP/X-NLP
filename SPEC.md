...
**结论**：前端 codegen 了 26 个方法，实际仅 12 个被页面使用。6 个 NLP 任务 API + Predict/Benchmark/Health 共 9 个已就绪但**无对应 UI**。
...
| 10 | 配置模型 | `AppConfig` / `ModelConfig` / `ModelType` / `ModelProtocol` / `ServerConfig` | ✅ | 标准模型类型与协议白名单 |
...
GET /api/v1/models/capabilities
    响应: { types: [CHAT, EMBEDDING, RERANKING], protocolsByType: {...}, requiredFields: {...} }
...
POST /api/v1/models
    请求: { name, type, protocol, provider, modelName, baseUrl?, apiKey?,
...
- 表单维护标准调用字段: type/protocol/provider/modelName/baseUrl/apiKey/maxInput/maxOutput
